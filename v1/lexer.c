...
int isHEX(FILE *tape) {
	...
	if(ch != 0) {
		ungetc(ch, tape); //retorna o valor lido
		return 0;
	}
	...
	headHEX = getc(tape); //foi necessario outra variavel para nao perder o valor anterior

	if(!isxdigit(headHEX)) { //verifica se é a-f, A-F, 0-9
...
int isEE(FILE *tape){
	...
	signal = getc(tape); //leio o sinal ou o valor da exponencial direto

	if(signal == '+' || signal =='-') { //se era sinal 

		numberExp = getc(tape); //recebo os numeros da exponencial
